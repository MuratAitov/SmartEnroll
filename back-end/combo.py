...
def get_merged_requirements_for_student(
    ...
    # =========================================================================
    # ШАГ 1. Собираем все секции, на которые студент записан
    # =========================================================================
    ...
    # Получаем (course_id, term, section, attribute)
    ...
    # Для Core логики нужен список словарей с term/section
    # Для Major логики — просто набор course_id
    taken_classes_core = []
    ...
    if not resp_core_prog.data:
        ...
        # Собираем (course_id, term, section) => {core_attrs, credits}
        ...
        for group in core_groups:
            ...
            if is_core_group:
                ...
                # Смотрим, какие (course_id, term, section) содержат такой атрибут
                ...
                # Для Core-групп оставляем строку вида "CORE 9" (пример)
                available_courses_val = f"CORE {required_credits}"
            else:
                # -------- НЕ CORE-группа (обычные требования)
                expected_attribute = None
                # potential courses (из requirement_courses)
                ...
                # Смотрим, какие из этих potential_codes студент взял
                for (c_id, t, s), info in taken_info_core.items():
                    if c_id in potential_codes:
                        allocated_courses.append(c_id)
                        total_taken_credits += info["credits"]

                # Вместо "3 potential courses" мы хотим вывести реальные коды
                # Но с учётом того, что студент уже взял часть.
                # Если хотим показать *все* потенциальные (включая ещё не взятые),
                # то вот так:
                potential_list = sorted(potential_codes)
                # Можно вычислить "ещё не взятые":
                # not_taken = potential_codes - set(allocated_courses)
                # Для демонстрации выведем просто *все* potential_codes
    ...
    # =========================================================================
    # ШАГ 3. ЛОГИКА ДЛЯ MAJOR (double count)
    # =========================================================================
    major_requirements = {}
    ...
    if not resp_major_prog.data:
        ...
        # Для Major логики: узнаём кредиты для всех taken_courses_major
        ...
        for group in major_groups:
            ...
            major_requirements[group_name] = {
                ...
                # Для Major-групп по-прежнему список кодов
    ...
    if full_view:
        ...
        for grp_name, info in core_requirements.items():
            ...
            # Если это список, перечислим все курсы
            # Если это строка, оставим как есть
    ...
    # =========================================================================
    # ШАГ 5. Возвращаем общий результат
    # =========================================================================
    return {
...
if __name__ == "__main__":
    # Пример: хотим объединить "University Core Requirements" + "B.S. Computer Science - Data Science Concentration"
    # для студента с ID=1
    result = get_merged_requirements_for_student(
        student_id=1,
        major_program_name="B.S. Computer Science - Data Science Concentration",
        core_program_name="University Core Requirements",  # по умолчанию
        full_view=True,
        debug=False
    )
